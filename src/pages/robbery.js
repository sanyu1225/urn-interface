...
import { debounce, isEmpty } from '@/plugin/lodash';
...
import { CREATOR_ADDRESS, getItemQuery, queryAllUrnData } from '../constant';
    ...
    <Layout>
      ...
      >
        ...
        >
          ...
          >
            <Text fontSize="20px" fontWeight={500} color="#FFF3CD" textAlign="center" w="100%" lineHeight="28px" whiteSpace="pre-line">
              Stop digging like a dumb ass, rob random fuckers make your life easier.
            </Text>
            <Flex w="100%" gap="16px" flexWrap={{ base: 'wrap', mid: 'initial' }}>
              <Button w="190px" onClick={() => robHandler('random')}>
                Rob a fucker
              </Button>
              <Button w="190px" onClick={() => robHandler('specific')}>
                Rob specific fucker
              </Button>
            </Flex>
          </Flex>
        ...
        >
          {robIsLoading ? (
            ...
            <>
              <Text color="#FFF5CE" fontSize={{ base: '20px' }} fontWeight={700} h="26px" mb="8px">
                Who robbed you?
              </Text>
              <Flex w="100%" flexWrap="wrap" maxH={{ base: '400px' }} overflow="auto" position="relative">
                {robdata?.length &&
                  robdata.map((item, index) => (
                    <Flex key={index} flexWrap="wrap" borderBottom="1px solid #383732" mt="12px">
                      <Flex justifyContent="flex-start" w="100%" key={index} h="20px" gap="12px">
                        ...
                        <Box cursor="pointer" onClick={() => copyToClipboard(item.data.robber)}>
        ...
        <Modal blockScrollOnMount={false} isOpen={isOpen} onClose={closeModalHandler}>
          ...
          <ModalContent p="28px" bg="#292229">
            ...
            <ModalBody p="0">
              ...
              <Flex bg="#211C21" w="100%" h={{ base: '140px' }} borderRadius="20px" justifyContent="center" alignItems="center" flexWrap="wrap">
                {isEmpty(UrnList) ? (
              ...
              {modalType === 'specific' && (
                <>
                  ...
                  <Input placeholder="0x..." onChange={(e) => setInputAddress(e.target.value)} />
                </>
              )}
              <Box w="100%" m="12px 0">
                <Text color="#FFF3CD" textAlign="left" fontSize="16px" fontWeight={700}>
